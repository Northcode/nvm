...
namespace nvm2.compiler
{
    class Parser
    {
        ...
        private string[] ParseClassImplemetations()
        {
            List<string> implemetations = new List<string>();
            do
            {
                if (Current == ',')
                    Step(); //Step over comma
                implemetations.Add(Current);
                Step(); //Step over string
            } while (!EOT && Current == ',');

            i--;

            return implemetations.ToArray();
        }
        ...
        private expr ParseExpr(bool CalledFromArithmeticParsing = false)
        {
            ...
            if(e == null)
            {
                if (Current == TokenType.word && Current != "end")
                {
                    e = ParseExprWord();
                }
            }
